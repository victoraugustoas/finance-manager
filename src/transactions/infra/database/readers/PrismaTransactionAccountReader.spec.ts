...
describe('PrismaTransactionAccountReader', () => {
  const accountId = '11111111-1111-1111-1111-111111111111';

  let findUnique: jest.Mock;
  let prisma: PrismaService;
  let acl: PrismaTransactionAccountReader;

  beforeEach(() => {
    findUnique = jest.fn();
    prisma = { account: { findUnique } } as unknown as PrismaService;
    acl = new PrismaTransactionAccountReader(prisma);
  });
  ...
  describe('existsById()', () => {
    ...
    it('should return REFERENCE_ACCOUNT_NOT_FOUND when no row is found', async () => {
      ...
      expect(result.errors[0].cls).toBe('PrismaTransactionAccountReader');
