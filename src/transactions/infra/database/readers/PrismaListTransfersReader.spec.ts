...
describe('PrismaListTransfersReader', () => {
  let findMany: jest.Mock;
  let query: PrismaListTransfersReader;
  ...
  beforeEach(() => {
    ...
    query = new PrismaListTransfersReader(prisma);
  });
  ...
  it('should return enriched transfer rows', async () => {
    ...
    const result = await query.read({ period: makePeriod() });
  ...
  it('should return PRISMA_QUERY_ERROR when findMany throws', async () => {
    ...
    expect(result.errors[0].cls).toBe('PrismaListTransfersReader');
  });
});
